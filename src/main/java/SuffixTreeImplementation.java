...
class SuffixTreeImplementation {
    ...
    SuffixTreeImplementation(String inputString) {
        String pattern = "(.)*\\$"; //the tree receives nly a certain type of strings
    ...
    private void addSuffix(String stringToAdd) {
        ...
        while (position < stringToAdd.length()) { //adding each symbol
            ...
            List<Integer> childrenOfCurrentNode = this.getNodes().get(currentNodeNumber).getChildren();//getting list of children of the current node
            ...
            while (true) {
                if (positionInSuffix == childrenOfCurrentNode.size()) { // if there are no matching child, remainder of stringToAdd becomes new node
                    ...
                    newNodeNumber = this.getNodes().size(); //giving an inserted node a number. which is the previous node`s number + 1
                ...
                // of the current node;
                if (this.getNodes().get(newNodeNumber).getEdge().charAt(0) == currentSymbol)
            ...
            String prefixOfSuffixRemained = this.getNodes().get(newNodeNumber).getEdge(); // finding prefix of remaining suffix in common with child
            ...
            while (i < prefixOfSuffixRemained.length()) {
                if (stringToAdd.charAt(position + i) != prefixOfSuffixRemained.charAt(i)) {// splitting Node with `newNodeNumber` number
                    ...
                    this.getNodes().get(currentNodeNumber).addChild(positionInSuffix, newNodeNumber);//adding new nodes to the current root
